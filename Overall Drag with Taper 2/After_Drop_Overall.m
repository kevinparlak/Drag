...
indexconstant=find(abs(v-205)<=10);
Vconstant=v(indexconstant);
Dconstant=D_tot(indexconstant);

%Battery weight used after drop
W_batt_drop=(Ebest/aircraft.e_batt)*world.g;
...
legend('front wing induced','rear wing induced','tail induced',...
    ...
    'fuselage','boom','landing gear','parasite','total','SPYRO Best L/D','SPYRO Constant Airspeed',...
    'Location','NorthWest')
ylabel('drag (lb)')
...
ylabel('L/D');
xlabel(speedlabel)
legend('LD profile','SPYRO After Drop')
...
legend('Power req','SPYRO After Drop')
subplot(2,1,2)
plot(v_u,LD,'b',Vbestknots,LDbest,'*')
ylabel('L/D');
xlabel(speedlabel)
...
ylabel('Drag (lb)')
legend('Total drag','SPYRO After Drop')
subplot(3,1,3)
plot(v,P.batt/550,v,P.aero/550,Vbest,Pbest/550,'*')
legend('Power avail (hp)','Power req (hp)','SPYRO After Drop')
ylabel('power (hp)')
