...
%-----INCREASED POWER-----
indexincrease=find(abs(P.aerohp-130)<=5);
Dincrease=D_tot(indexincrease);                           %lb
Vincrease=v(indexincrease);                               %ft/s
Pincrease=P.aero(indexincrease);                          %lb-ft/s
Eincrease=Dincrease*range/etaprop;                        %lb-ft
LDincrease=LD(indexincrease);
Wincrease=(Eincrease/aircraft.e_batt)*world.g;            %lb
...
Vincreaseknots=v_u(indexincrease);
...
plot(v_u,D_ind(1,:)+D_ind(2,:),'r',...
    ...
    v_u,D_tot,'k',...
    Vbestknots,Dbest,'*',...
    Vincreaseknots,Dincrease,'o');
legend('front wing induced','rear wing induced','tail induced',...
    'front wing profile','rear wing profile','tail profile',...
    'fuselage','boom','landing gear','parasite','total','Minimum Drag','Increased Power','Location','NorthWest')
ylabel('Drag (lb)')
...
%-----LD-----
figure
plot(v_u,LD,'b',Vbestknots,LDbest,'*',Vincreaseknots,LDincrease,'o')
ylabel('L/D');
xlabel(speedlabel)
legend('LD profile','Minimum Drag','Increased Power');

%-----AERODYNAMIC POWER REQUIRED-----
figure
ha2=subplot(2,1,1);
plot(v_u,P.aero/550,'b',Vbestknots,P.aero(indexbest)/550,'*',Vincreaseknots,P.aero(indexincrease)/550,'o')
ylabel('Aero power required (hp)')
legend('Power required (hp)','Minimum Drag','Increased Power')
subplot(2,1,2)
plot(v_u,LD,'b',Vbestknots,LDbest,'*',Vincreaseknots,LDincrease,'o')
ylabel('L/D');
xlabel(speedlabel)
...
subplot(3,1,1)
plot(v_u,LD,Vbestknots,LDbest,'*',Vincreaseknots,LDincrease,'o')
ylabel('L/D')
legend('LD profile','Minimum Drag','Increased Power')
subplot(3,1,2)
plot(v_u,D_tot,Vbestknots,Dbest,'*',Vincreaseknots,Dincrease,'o')
ylabel('Drag (lb)')
legend('Total drag','Minimum Drag','Increased Power')
subplot(3,1,3)
plot(v_u,P.aero/550,Vbestknots,Pbest/550,'*',Vincreaseknots,Pincrease/550,'o')
legend('Power required (hp)','Minimum Drag','Increased Power')
ylabel('Power (hp)')
xlabel('Airspeed (knots)')
axis([100,400,0,250])
...
fprintf('Airspeed to cruise (knots) = %f\n',Vbestknots)
